...
datalist = vector("list", length = 7)
for(i in directories){
  one = (paste0('output/FarmCPU_2025/', i, '/GAPIT.Association.Filter_GWAS_results.csv'))
  one <- read.csv(one, header=TRUE)
  i <- paste0(i)
  one$Year = i #add year
  one$Trait = gsub('^.*.', '', one$traits) #split string and extract trait name
...
big_data = do.call(rbind, datalist)
rownames(big_data) <- NULL 
big_data$ID <- gsub("\\(NYC\\)", "", big_data$ID)

# write.csv(big_data, 'data/Intermediate_Files/SigM_2.csv', row.names = FALSE)
Sig <- read.csv('data/Intermediate_Files/SigM.csv', head=TRUE)
#Sig <- read.csv('data/Intermediate_Files/SigM_2.csv', head=TRUE)
...
data <- Sig[! str_detect(Sig$id, "ERGSEV"),] # remove ERGSEV
data <- data[! str_detect(data$id, "BLSSEV"),] # remove ERGSEV
data <- data[! str_detect(data$id, "GLBLSEV"),] # remove ERGSEV
data <- data[! str_detect(data$id, "HSATIVLSEV"),] # remove ERGSEV
rownames(data) <- NULL
...
dfMarkPos2$markName <- gsub("\\_C.*","",dfMarkPos2$markName) #remove characters after the trait name
dfMarkPos2 <- dfMarkPos2[,c('chrName', 'markPos', 'markSize', 'markName')] # reorder
dfMarkPos2 <- dfMarkPos2[! str_detect(dfMarkPos2$markName, "ERGSEV"),] # remove ERGSEV
dfMarkPos2 <- dfMarkPos2[! str_detect(dfMarkPos2$markName, "BLSSEV"),] # remove ERGSEV
dfMarkPos2 <- dfMarkPos2[! str_detect(dfMarkPos2$markName, "GLBLSEV"),] # remove ERGSEV
dfMarkPos2 <- dfMarkPos2[! str_detect(dfMarkPos2$markName, "HSATIVLSEV"),] # remove ERGSEV
...
marks4 <- marks3[!str_detect(marks3$Type, "ERGSEV"),] #remove trait from list of significant markers
marks4 <- marks4[!str_detect(marks4$Type, "BLSSEV"),] #remove trait from list of significant markers
marks4 <- marks4[!str_detect(marks4$Type, "GLBLSEV"),] #remove trait from list of significant markers
marks4 <- marks4[!str_detect(marks4$Type, "HSATIVLSEV"),] #remove trait from list of significant markers
colnames(marks4)[3] <- "Trait"
```

```{r}

myColors2 <- c("#b6dbff", "#006ddb", "#6db6ff", "#004949", "#009292", "#db6d00", "#999933", "#CC6677", "#ffb6db" , "#920000", "#924900", "#DDCC77") # in alphabetical order
# "#490092" BLSSEV , "#332288" GLBLSEV , "#b66dff" HSATIVLSEV
names(myColors2) <- levels(marks4$Trait)
colScale2 <- scale_colour_manual(name = "Trait",values = myColors2)
...
# Export > Save as PDF.. > Directory= '~output/Figures' ; File Name = Map_of_all_Sig_Markers_by_group_legend2_2025
# orientation: Portrait 
# dimensions: 5.75x5.36
