...
FHB_for_GS_GWAS <- read.table(here(file = 'data','Original_Files','FHB_for_GS_GWAS.txt'), header = TRUE, row.names = 1, stringsAsFactors = FALSE) #load genotype calls
...
GM <- read.table(here('data','Intermediate_Files','SNP_Position.txt'), head=TRUE)
...
FHB1 <- read.csv(here('data','Intermediate_Files','FHB1_markers.csv'))
```
...
main_dirr <- 'output/FarmCPU_2025/C07_SAL_2018/GAPIT.Association.GWAS_Results.FarmCPU.' #designate directory path 

for(i in traits){
  two = (paste0(main_dirr, i, '(NYC).csv'))
  two <- read.csv(two)
...
#Set up and run loop to read in output tables for disease related traits and assign a year_trait name
traits <- c("FHBD3G.DON", "FHBDISIND", "FHBDON", "FHBSPKINC", "FHBD3G")
main_dirr <- 'output/FarmCPU_2025/C07_SAL_2019/GAPIT.Association.GWAS_Results.FarmCPU.' #designate directory path 
...
main_dirr <- 'output/FarmCPU_2025/C07_SAL_2020/GAPIT.Association.GWAS_Results.FarmCPU.' #designate directory path 
...
#Set up and run loop to read in output tables for disease related traits and assign a year_trait name
traits <- c("FHBDISIND", "FHBDON", "FHBSPKINC")
main_dirr <- 'output/FarmCPU_2025/C10_OLA_2021/GAPIT.Association.GWAS_Results.FarmCPU.' #designate directory path 
...
main_dirr <- 'output/FarmCPU_2025/C10_OLA_2022/GAPIT.Association.GWAS_Results.FarmCPU.' #designate directory path 
...
#Set up and run loop to read in output tables for disease related traits and assign a year_trait name
traits <- c("FHBDISIND", "FHBDON", "FHBSPKINC")
main_dirr <- 'output/FarmCPU_2025/C10_SAL_2021/GAPIT.Association.GWAS_Results.FarmCPU.' #designate directory path 
...
#Set up and run loop to read in output tables for disease related traits and assign a year_trait name
traits <- c("FHBZEA", "FHBDON", "FHBDISIND", "FHBSPKINC")
main_dirr <- 'output/FarmCPU_2025/C10_SAL_2022/GAPIT.Association.GWAS_Results.FarmCPU.' #designate directory path 
...
threshold <- rbind(threshold_0.10_2018, threshold_0.10_2019, threshold_0.10_O2021, threshold_0.10_O2022, threshold_0.05_2018, threshold_0.05_2019, threshold_0.05_O2022, threshold_0.01_O2022)
...
#### Then go to jBrowser and find each region
# https://phytozome-next.jgi.doe.gov/jbrowse/index.html?data=genomes%2FTintermedium_v3_1&loc=J03%3A51106461..51106610&tracks=Transcripts%2CAlt_Transcripts%2CPASA_assembly%2CBlatx_Grass%2CBlastx_protein%2CDNA%2Clifted%2CMapped%20Models%20From%20Tintermedium_v2_1%2CBlatx_BasalMonocot%2CBlatx_Dicots&highlight=
  
threshold$T.inter_Genes <- c("No IWG transcript", "Thint.S03G047000 (PAC:55872180)", "No IWG transcript", "Thint.S03G047000 (PAC:55872180)", "Thint.S03G047000 (PAC:55872180)", "Thint.J03G037000 (PAC:55768806)", "No IWG transcript", "Thint.S03G047000 (PAC:55872180)", "No IWG transcript", "Thint.J03G037000 (PAC:55768806)", "Thint.J03G037000 (PAC:55768806)")
threshold$T.inter_Description <- c(" ", "SGNH hydrolase, GDSL lipase/esterase; hydrolase activity, acting on ester bonds", " ", "SGNH hydrolase, Alpha-L-fucoside fucohydrolase, GDSL lipase/esterase; hydrolase activity, acting on ester bonds", "SGNH hydrolase, Alpha-L-fucoside fucohydrolase, GDSL lipase/esterase; hydrolase activity, acting on ester bonds", "SGNH hydrolase, Alpha-L-fucoside fucohydrolase, GDSL lipase/esterase; hydrolase activity, acting on ester bonds", " ", "SGNH hydrolase, GDSL lipase/esterase; hydrolase activity, acting on ester bonds", " ", "SGNH hydrolase, Alpha-L-fucoside fucohydrolase, GDSL lipase/esterase; hydrolase activity, acting on ester bonds", "SGNH hydrolase, Alpha-L-fucoside fucohydrolase, GDSL lipase/esterase; hydrolase activity, acting on ester bonds")

# download the reference sequence for those regions
# BLAST to : T. aestivum (taxa id: 4565), T. dicoccoides (85692), H. vulgare (4513). Hits with query cover >80% were considered significant.  
# Download Description Table (csv)

threshold$NCBI_Description <- c("Triticum aestivum chromosome 3B-specific BAC library", "Sumai 3 Fhb1 region", "Triticum aestivum chromosome arm 3DS-specific BAC library", "Sumai 3 Fhb1 region", "Sumai 3 Fhb1 region", "Sumai 3 Fhb1 region", "Triticum aestivum chromosome 3B-specific BAC library", "Sumai 3 Fhb1 region", "Triticum aestivum chromosome arm 3DS-specific BAC library", "Sumai 3 Fhb1 region", "Sumai 3 Fhb1 region")
threshold$NCBI_Species <- c("T.aestivum", "T.aestivum", "T.aestivum", "T.aestivum", "T.aestivum", "T.aestivum", "T.aestivum", "T.aestivum", "T.aestivum", "T.aestivum", "T.aestivum")
threshold$Query <- c("88", "90", "100", "74", "74", "100", "88", "90", "100", "100", "100")
threshold$Per.Ident <- c("91.01", "95", "85.57", "95.97", "95.97", "92.17", "91.01", "95", "85.57", "92.17", "92.17")

# write.csv(threshold, 'data/Final_Files/FHB1_sig.markers_adjusted_threshold.csv')
...
rm(J03,V03,S03,GM)
rm(`2022O_FHBDON`,`2022O_FHBZEA`,`2022S_FHBDON`,`2022S_FHBZEA`)
```
